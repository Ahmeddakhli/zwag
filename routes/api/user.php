...
Route::prefix('v1/user')->name('api.v1.user.')->group(function () {
    ...
    Route::namespace('User\Auth')->middleware('guest:sanctum')->group(function () {

        Route::controller('LoginController')->group(function () {
            Route::get('/login', 'showLoginForm')->name('login');
            Route::post('/login', 'login');
            Route::get('logout', 'logout')->middleware('auth:sanctum')->withoutMiddleware('guest:sanctum')->name('logout');
        });

        Route::controller('RegisterController')->group(function () {
            Route::get('register', 'showRegistrationForm')->name('register');
            Route::post('register', 'register');
            Route::post('check-user', 'checkUser')->name('checkUser')->withoutMiddleware('guest:sanctum');
        });
    ...
    Route::middleware(['auth:sanctum'])->group(function () {
        ...
        Route::post('marital-statuses', 'Admin\MaritalStatusController@store');

        Route::post('user-data-submit/{step?}', 'User\UserController@userDataSubmit')->name('data.submit');
        Route::namespace('Admin')->controller(LookupController::class)->prefix('admin/lookups')->group(function () {
            Route::get('/', 'index')->name('admin.lookups.index');
            Route::post('/', 'store')->name('admin.lookups.store');
            Route::get('/{id}', 'show')->name('admin.lookups.show');
            Route::put('/{id}', 'update')->name('admin.lookups.update');
            Route::delete('/{id}', 'destroy')->name('admin.lookups.destroy');
            Route::get('/slug/{slug}', 'getBySlug')->name('admin.lookups.by-slug');
            Route::get('/parent/{parent_id}', 'index')->name('admin.lookups.by-parent');
        });
