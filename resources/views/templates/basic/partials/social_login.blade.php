...
@endphp
@if ($credentials->google && $credentials->google->status == App\Constants\Status::CUR_BOTH)ENABLE || $credentials->facebook->status == App\Constants\Status::CUR_BOTH)ENABLE || $credentials->linkedin->status == App\Constants\Status::CUR_BOTH)ENABLE)
    <div class="registration-socails mt-3">
        ...
        <ul class="registration-socails__list social-icons list list--row justify-content-center">
            @if ($credentials->google->status == App\Constants\Status::CUR_BOTH)ENABLE)
            ...
            @endif
            @if ($credentials->facebook->status == App\Constants\Status::CUR_BOTH)ENABLE)
            ...
            @endif
            @if ($credentials->linkedin->status == App\Constants\Status::CUR_BOTH)ENABLE)
