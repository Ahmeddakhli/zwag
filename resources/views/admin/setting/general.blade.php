                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-xl-3 col-sm-6">
                                <label>@lang('Default Package')
                                    <span class="text--primary" title="@lang('If you select any of these, it is set as default package and automatically added to the new user profile while he registered.')"><i class="fas fa-info-circle"></i></span>
                                </label>
                                <select class="form-control" name="default_package">
                                    <option value="">@lang('Select One')</option>
                                    @foreach ($packages as $package)
                                        <option @if (gs('default_package_id') == $package->id) selected @endif value="{{ $package->id }}">{{ __($package->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-xl-3 col-sm-6 ">
                                ...
                                <select class="select2 form-control" name="currency_format" data-minimum-results-for-search="-1">
                                    <option value="1" @selected(gs('currency_format') == App\Constants\Status::CUR_BOTH)CUR_BOTH)>@lang('Show Currency Text and Symbol Both')</option>
                                    <option value="2" @selected(gs('currency_format') == App\Constants\Status::CUR_BOTH)CUR_TEXT)>@lang('Show Currency Text Only')</option>
                                    <option value="3" @selected(gs('currency_format') == App\Constants\Status::CUR_BOTH)CUR_SYM)>@lang('Show Currency Symbol Only')</option>
                                </select>
                            </div>
                        </div>
